...
/*   Updated: 2023/10/19 09:34:59 by albaud           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ia_forward(t_net *net, const t_v *input)
{
	...
	// v_apply(&net->hiden[0], net->info.function);
	v_addapply(&net->hiden[0], &net->bias[0], net->info.function);
	i = 0;
	while (++i < net->info.hiden_layers)
	{
		...
		v_addapply(&net->hiden[i], &net->bias[i], net->info.function);
	}
	m_hvdot(&net->weights[i], &net->hiden[i - 1], &net->predicted_output);
	// v_add(&net->predicted_output, &net->bias[i], &net->predicted_output);
	// v_apply(&net->predicted_output, net->info.function);
	v_addapply(&net->predicted_output, &net->bias[i], net->info.function);
}
